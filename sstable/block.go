...
type blockIter struct {
	...
	// assertNoSuffixes, if set to true in an invariants build, checks that every
	// surfaced key does not contain a suffix
	assertNoSuffixes bool
}
...
func newBlockIter(
	...
	i := &blockIter{}
	return i, i.init(cmp, split, block, 0, false, syntheticPrefix, syntheticSuffix)
}
...
func (i *blockIter) initHandle(
	...
	i.handle.Release()
	i.handle = block
	return i.init(cmp, split, block.Get(), globalSeqNum, hideObsoletePoints, syntheticPrefix, syntheticSuffix)
}
...
// in-place replacement. When maybeFromCache is true, to prevent modifying the
...
func (i *blockIter) maybeReplaceSuffix(maybeFromCache bool) {
	...
	if invariants.Enabled && i.assertNoSuffixes {
		prefixLen := i.split(i.ikey.UserKey)
		if prefixLen != len(i.ikey.UserKey) {
			panic("key has a suffix but assertNoSuffixes==true")
		}
	}
}
